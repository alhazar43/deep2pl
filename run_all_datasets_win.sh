#!/bin/bash
# run_all_datasets_win.sh - Comprehensive Deep-2PL Pipeline Runner for All Datasets (Windows Compatible)
#
# This script is adapted for Windows and should be run using a bash interpreter (e.g., from Git Bash).
# It runs the complete pipeline for ALL datasets:
...
#
# Usage:
#   From a bash terminal (like Git Bash):
#   ./run_all_datasets_win.sh [OPTIONS]
#
# Note: This script calls 'run_dataset.sh'. Ensure you have a version of that script
# that is also compatible with your Windows bash environment.
...
estimate_time() {
    ...
    for dataset in "${datasets[@]}"; do
        if [[ -n "${TIME_ESTIMATES[$dataset]}" ]]; then
            ...
            # Scale by epochs
            dataset_time=$(python -c "print(round($dataset_time * $EPOCHS / 30, 2))")
            
            # Scale for single fold
            if [[ "$SINGLE_FOLD" == true ]]; then
                dataset_time=$(python -c "print(round($dataset_time / 5, 2))")
            fi
            
            total_hours=$(python -c "print($total_hours + $dataset_time)")
            echo "    $dataset: ${dataset_time}h"
        fi
    done
    
    total_hours=$(python -c "print(round($total_hours, 2))")
    echo "    Total estimated time: ${total_hours}h"
    
    is_long=$(python -c "print(1 if float($total_hours) > 8 else 0)")
    if (( is_long )); then
...
run_dataset_pipeline() {
    ...
    # Build command, explicitly using bash to run the script for Windows compatibility
    local cmd="bash ./run_dataset.sh $dataset --epochs $EPOCHS"
...
if ! command -v python &> /dev/null; then
    print_error "python not found. Please ensure python is in your PATH."
    exit 1
fi
