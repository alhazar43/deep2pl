"""Deep-IRT model with optional per-KC tracking based on Q-matrix availability.
Supports both global and per-KC modes with continuous evolution.
"""
...
class DeepIRTModel(nn.Module):
    """Deep-IRT model with optional per-KC tracking."""
    
    def __init__(self, n_questions, memory_size=50, key_dim=50, 
                 value_dim=200, summary_dim=50, 
                 q_embed_dim=None, qa_embed_dim=None, ability_scale=3.0, 
                 use_discrimination=True, dropout_rate=0.0, 
                 q_matrix_path=None, skill_mapping_path=None):
        """Initialize Deep-IRT model.
        Args: n_questions, memory_size, key_dim, value_dim, summary_dim, 
              ability_scale, use_discrimination, dropout_rate, q_matrix_path
        """
        super(DeepIRTModel, self).__init__()
        
        # Configure embedding dimensions with defaults
        if q_embed_dim is None:
            q_embed_dim = key_dim
        if qa_embed_dim is None:
            qa_embed_dim = value_dim
        ...
        summary_input_dim = value_dim + q_embed_dim
        ...
        if self.per_kc_mode:
            ...
            self.kc_cross_net = nn.Sequential(
                nn.Linear(self.n_kcs * self.kc_state_dim, summary_dim),
                nn.Tanh(),
                nn.Linear(summary_dim, self.n_kcs * self.kc_state_dim),
                nn.Dropout(dropout_rate)
            )
            
            # Initialize item difficulty estimation with KC context
            self.item_difficulty_net = nn.Sequential(
                nn.Linear(q_embed_dim + self.n_kcs, summary_dim),
                nn.Tanh(),
                nn.Linear(summary_dim, 1)
            )
            
            # Initialize learnable initial KC states
            self.kc_init = nn.Parameter(
            ...
            self.item_difficulty_net = nn.Sequential(
                nn.Linear(q_embed_dim, summary_dim),
                nn.Tanh(),
                nn.Linear(summary_dim, 1)
            )
    ...
    def _init_weights(self):
        ...
        if self.per_kc_mode:
            nn.init.kaiming_normal_(self.kc_init)
    ...
    def forward(self, q_data, qa_data, target_mask=None):
        ...
        if self.per_kc_mode:
            ...
            for kc_id in range(self.n_kcs):
                kc_states[kc_id] = self.kc_init[kc_id].unsqueeze(0).repeat(batch_size, 1)
        ...
        for t in range(seq_len):
            ...
            # Memory operations
            attn_weight = self.memory.attention(q_t)
            read_content = self.memory.read(attn_weight)
            ...
            if self.per_kc_mode:
                ...
                cross_kc_influence = self.kc_cross_net(all_kc_states_flat)
            ...
            # Update memory
            self.memory.write(attn_weight, qa_t)
