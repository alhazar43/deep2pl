...
class MemoryHeadGroup(nn.Module):
    ...
    def read(self, value_memory_matrix, correlation_weight):
        ...
        read_content = read_content.squeeze(1)  # (batch_size, value_memory_state_dim)
    ...
    def write(self, value_memory_matrix, correlation_weight, embedded_content_vector):
        ...
        erase_signal_expanded = erase_signal.unsqueeze(1)  # (batch_size, 1, value_memory_state_dim)
        add_signal_expanded = add_signal.unsqueeze(1)      # (batch_size, 1, value_memory_state_dim)
    ...
    def __init__(self, memory_size, key_memory_state_dim, value_memory_state_dim, 
        ...
        if init_key_memory is not None:
            ...
            self.key_memory_matrix = nn.Parameter(
                torch.randn(memory_size, key_memory_state_dim)
            )
            nn.init.kaiming_normal_(self.key_memory_matrix)
        
        # Memory head groups
        self.key_head = MemoryHeadGroup(memory_size, key_memory_state_dim, is_write=False)
        self.value_head = MemoryHeadGroup(memory_size, value_memory_state_dim, is_write=True)
        ...
        if init_value_memory is not None:
            if init_value_memory.dim() == 2:
                ...
                self.value_memory_matrix = init_value_memory.unsqueeze(0).expand(
                    batch_size, self.memory_size, self.value_memory_state_dim
                ).contiguous()
            else:
